          ...
          builder: (context, snapshot) {
            ...
            return CustomScrollView(
              slivers: [
                SliverAppBar(
                  expandedHeight:
                      400.0, // Meningkatkan tinggi untuk gambar penuh
                  pinned: true,
          ...
          child: FutureBuilder<masterBook>(
            ...
            builder: (context, snapshot) {
              if (snapshot.connectionState == ConnectionState.waiting) {
                ...
                // Debugging output
                //print('Sinopsis: ${book.sinopsis}');

                return Row(
                  mainAxisAlignment: MainAxisAlignment.spaceBetween,
                  children: [
                    if (book.sinopsis != null &&
                        book.sinopsis!.isNotEmpty &&
                      ...
                      ElevatedButton.icon(
                        ...
                        label: const Text('run automate'),
                        style: ElevatedButton.styleFrom(
                          padding: const EdgeInsets.symmetric(
                              horizontal: 24, vertical: 12),
                          backgroundColor:
                              const Color.fromARGB(255, 37, 198, 1),
                          textStyle: const TextStyle(fontSize: 16),
                        ),
                      ),
                    ],
                    // Show Add Sinopsis button if sinopsis is null, empty, or contains "No synopsis available"
                    if (book.sinopsis == null ||
                        book.sinopsis!.isEmpty ||
                        book.sinopsis == "No synopsis available")
                      ElevatedButton.icon(
                        onPressed: () {
                          Navigator.push(
